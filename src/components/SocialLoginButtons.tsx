...
type Provider = 'google' | 'azure' | 'apple' | 'github';
        ...
        {PROVIDERS.map((provider) => {
          ...
          return (
            <button
              ...
              disabled={isDisabled}
              aria-label={`${label} ${provider.label}`}
              className="flex items-center justify-center gap-2 px-3 py-2 border border-secondary-300 rounded-lg text-sm text-secondary-700 bg-white hover:bg-secondary-50 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {isLoading ? (
                <svg className="animate-spin h-4 w-4 text-secondary-400" aria-hidden="true" viewBox="0 0 24 24">

...
// FedRAMP High baseline resolved catalog (contains only High-baseline controls)
const OSCAL_URL = 'https://raw.githubusercontent.com/usnistgov/oscal-content/main/nist.gov/SP800-53/rev5/json/NIST_SP-800-53_rev5_HIGH-baseline-resolved-profile_catalog.json';
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { data: framework, error: fwError } = await supabase
      ...
      .select('id')
      .eq('abbreviation', 'FedRAMP High')
      .single();

    if (fwError || !framework) {
      return new Response(
        JSON.stringify({ error: 'FedRAMP High framework not found — run seed SQL first', details: fwError?.message }),
    ...
    if (!source) {
      return new Response(
        JSON.stringify({ error: 'FedRAMP High source not found — run seed SQL first' }),
        { status: 404, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }
    ...
    for (const group of groups) {
      ...
      const familyContent = [
        `# FedRAMP High — ${familyAbbr}: ${familyName}`,
        ``,
        `Control Family: ${familyName} (${familyAbbr})`,
        `Framework: FedRAMP High Baseline (NIST SP 800-53 Rev 5)`,
        ``,
        `## Controls Required at FedRAMP High Baseline`,
        ...controls.map(c => {
      ...
      await insertDocument({
        ...
        title: `FedRAMP High — ${familyAbbr}: ${familyName}`,
        rawContent: familyContent,
        metadata: { family_id: familyAbbr, family_name: familyName, document_level: 'family', baseline: 'high' },
